...
(function (win) {
    ...
    function updateTokenDisplay() {
        ...
        if (tokens.length === 0) {
            html = '<tr><td colspan="6" class="v2-token-empty">暂无记录</td></tr>';
        }
        tbody.innerHTML = html;
    ...
    function createControlPanel() {
        ...
        panel.innerHTML =
            ...
            '<thead><tr><th>#</th><th>ticket</th><th>套餐</th><th>时间</th><th>preview</th><th>锁单</th></tr></thead>' +
            '<tbody id="v2-token-tbody"><tr><td colspan="6" class="v2-token-empty">暂无预存</td></tr></tbody>' +
            '</table>' +
            '</div>' +
            ...
            '</label>' +
            '<label class="v2-checkbox-label">' +
            '<input type="checkbox" style="display:none;" id="v2-throttle"' + (savedThrottle === 'true' ? ' checked' : '') + ' />' +
            '控流' +
            '</label>' +
            '<label class="v2-checkbox-label">' +
            ...
            '调剂模式' +
            '</label>' +
            '<button id="v2-alt-help" class="v2-help-btn" title="调剂说明">?</button>' +
            '</div>' +
            '<div class="v2-separator"></div>' +
        ...
        document.getElementById('v2-start-btn').addEventListener('click', function () {
            ...
            if (state.running) {
                ...
                startRefreshLockout();   // 进入 30s 锁定窗口：强制非售罄、禁手动刷新、不终止
